    ...
    <div class="wrap">
      ...
      <?php esc_html_e('These settings are optional and should usually be used only on a fresh installation', 'stratus'); ?>
        ...
        <div class="theme-activation-options theme-activation-step-two">
            ...
            <?php
            $themo_demo_masterslider_import_completed = get_option( 'themo_demo_masterslider_import_completed' );
            if (isset($themo_demo_masterslider_import_completed) && $themo_demo_masterslider_import_completed == 1){ ?>
                <p class="dashicons-before dashicons-yes"><?php esc_html_e('Master Slider import completed', 'stratus'); ?></p>
            <?php }else{ ?>
                <p class="dashicons-before dashicons-download"><?php printf(wp_kses_post(__('<strong>%1$sImport Master Slider Content%2$s</strong> - "%3$s", includced in theme zip.', 'stratus')), '<a href="'.esc_url(get_admin_url()).'admin.php?page=masterslider'.'">','</a>',wp_get_theme().'-master-slider.json'); ?></p>
            <?php } ?>
            ...
            <?php }else{ ?>
                <p class="dashicons-before dashicons-download"><?php printf(wp_kses_post(__('<strong>%1$sImport Demo WooCommerce Content%2$s</strong>  - "%3$s", included in theme zip.', 'stratus')), '<a href="'.esc_url(get_admin_url()).'admin.php?import=wordpress'.'">','</a>','woo-dummy-data.xml'); ?></p>
                <p style="padding:10px; background-color:#e7e7e7; max-width:600px" class="dashicons-before dashicons-clock"><?php printf(wp_kses_post(__('Importing  takes time. %sLearn more.%s', 'stratus')), '<a href="http://docs.themovation.com/stratus/#democontent'.'" target="_blank">','</a></em>'); ?></p>
            <?php }
            ?>
