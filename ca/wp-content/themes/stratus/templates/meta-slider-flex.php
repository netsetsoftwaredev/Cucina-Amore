...
if($show == 1) { // Slider Enabled
	...
	if ($loop->have_posts()) {
		...
		while ($loop->have_posts()) {
            ...
            // Text Alignment
            $text_align = false;
            if(isset($element["_text_align"][0]) && $element["_text_align"][0] > '') {
                $text_align = $element["_text_align"][0].' ';
            }
			...
			echo '<li>';
			echo '<div class="slider-bg ', $text_align, $content_align, $form_border, $text_contrast_class,' ', $key,'_',$i,'" ', $slider_bg_css,'>';
