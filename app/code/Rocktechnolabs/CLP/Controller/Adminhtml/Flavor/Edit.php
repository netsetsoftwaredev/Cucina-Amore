...
class Edit extends \Magento\Backend\App\Action
{
    ...
    public function __construct(
        ...
        \Rocktechnolabs\CLP\Model\Flavor $productFlavor
    )
    {
    ...
    public function execute()
    {
        ...
        $this->_coreRegistry->register('flavor_data', $model);

        // 5. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Flavor') : __('New Flavor'),
            $id ? __('Edit Flavor') : __('New Flavor')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Flavor'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? __('Edit Flavor') : __('New Flavor'));
        return $resultPage;
    }
}
