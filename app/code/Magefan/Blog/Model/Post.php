 ...
 * @method string _getMetaDescription()
...
class Post extends \Magento\Framework\Model\AbstractModel {
    ...
    \Magento\Framework\Model\Context $context, \Magento\Framework\Registry $registry, \Magento\Framework\Math\Random $random, \Magento\Cms\Model\Template\FilterProvider $filterProvider, \Magento\Framework\App\Config\ScopeConfigInterface $scopeConfig, Url $url, \Magefan\Blog\Model\ImageFactory $imageFactory, \Magefan\Blog\Model\AuthorFactory $authorFactory, \Magefan\Blog\Model\ResourceModel\Category\CollectionFactory $categoryCollectionFactory, \Magefan\Blog\Model\ResourceModel\Tag\CollectionFactory $tagCollectionFactory, \Magefan\Blog\Model\ResourceModel\Comment\CollectionFactory $commentCollectionFactory, \Magento\Catalog\Model\ResourceModel\Product\CollectionFactory $productCollectionFactory, \Magento\Framework\Model\ResourceModel\AbstractResource $resource = null, \Magento\Framework\Data\Collection\AbstractDb $resourceCollection = null, array $data = []
    ) {
    ...
    public function setGalleryImages(array $images) {
        $this->setData('media_gallery', implode(
                        self::GALLERY_IMAGES_SEPARATOR, $images
                )
        );
    ...
    public function getGalleryImages() {
        if (!$this->hasData('gallery_images')) {
            $images = array();
            $gallery = explode(
                    self::GALLERY_IMAGES_SEPARATOR, $this->getData('media_gallery')
            );
            if (!empty($gallery)) {
            ...
            if ($this->getShortContent()) {
                ...
                if (!$p) {
                    $p = (int) $this->scopeConfig->getValue(
                                    'mfblog/post_list/shortcotent_length', \Magento\Store\Model\ScopeInterface::SCOPE_STORE
                    );
                }
    ...
    public function getDirectionsContent() {
        $html = $this->filterProvider->getPageFilter()->filter($this->getData('directions_content'));
        return $html;       
    }
    
    public function getIngredientsContent() {
        $html = $this->filterProvider->getPageFilter()->filter($this->getData('ingredients_content'));
        return $html;       
    }
    public function getSuggestionsContent() {
        $html = $this->filterProvider->getPageFilter()->filter($this->getData('suggestions_content'));
        return $html;       
    }
    ...
    public function getRelatedPosts() {
        if (!$this->hasData('related_posts')) {
            ...
            $collection->getSelect()->joinLeft(
                    ['rl' => $this->getResource()->getTable('magefan_blog_post_relatedpost')], 'main_table.post_id = rl.related_id', ['position']
            )->where(
                    'rl.post_id = ?', $this->getId()
            );
    ...
    public function getRelatedProducts() {
        if (!$this->hasData('related_products')) {
            ...
            $collection->getSelect()->joinLeft(
                    ['rl' => $this->getResource()->getTable('magefan_blog_post_relatedproduct')], 'e.entity_id = rl.related_id', ['position']
            )->where(
                    'rl.post_id = ?', $this->getId()
            );
    ...
    public function isVisibleOnStore($storeId) {
        return $this->getIsActive() && $this->getData('publish_time') <= $this->getResource()->getDate()->gmtDate() && array_intersect([0, $storeId], $this->getStoreIds());
    }
    ...
    public function getPublishDate($format = 'Y-m-d H:i:s') {
        return \Magefan\Blog\Helper\Data::getTranslatedDate(
                        $format, $this->getData('publish_time')
        );
    }
    ...
    public function getUpdateDate($format = 'Y-m-d H:i:s') {
        return \Magefan\Blog\Helper\Data::getTranslatedDate(
                        $format, $this->getData('update_time')
        );
    }
    ...
    public function getSecret() {
        if ($this->getId() && !$this->getData('secret')) {
            $this->setData(
                    'secret', $this->random->getRandomString(32)
            );
            $this->save();
        }
