...
class View extends \Magefan\Blog\App\Action\Action {
    ...
    public function execute() {
        ...
        $tableName = $resource->getTableName('magefan_blog_post');
        $connection = $resource->getConnection();
        $sql = "Update " . $tableName . " Set views_count = IFNULL(views_count, 0) + 1 where post_id =" . $post->getId();
        $connection->query($sql);
        //$connection->update('magefan_blog_post', ['views_count' => new \Zend_Db_Expr('views_count + 1'),], ["post_id = ?" => $post->getId()]);
        /*         * **END post vister*** */
        return $resultPage;
    }
